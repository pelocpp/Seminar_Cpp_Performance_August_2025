...
// #define FALSE_SHARING
        ...
        struct resultType
        {
            int val[32];      // im besten Fall: 4 Bytes
        };
#else
        std::cout << "Without false sharing\n";
        struct resultType
        {
            alignas(std::hardware_destructive_interference_size) int val[32];
        };
#endif
                ...
                threads[i] = std::thread{
                    [&result]() mutable {
                        for (std::size_t j = 0; j < NumIterations; ++j) {
                            result.val[0] = (result.val[0] + std::rand() % 10) % 50;
                            result.val[31] = (result.val[31] + std::rand() % 10) % 50;
        ...
        const auto res = std::accumulate(
            ...
            [](const resultType a, const resultType b) {
                auto s = a.val[0] + b.val[0];
                return resultType{ s };
            }
        );

        std::println("Result:       {}", res.val[0]);
    }
}
